            ...
            GeometryReader { _ in
                ZStack {
                    ShowGuaranteeView(animation: animation, cards: $cards, showSendSheet: $showSendSheet, showWalletSheet: $showWalletSheet, clickBan: $clickBan)
                        ...
                        .opacity(tabData.currentTab == resellerTabs[0] ? 1 : 0)
                    ShowApprovedView(animation: animation, approved_cards: $approved_cards, showSendSheet: $showSendSheet, clickBan: $clickBan)
                        .environmentObject(tabData)
                        .opacity(tabData.currentTab == resellerTabs[1] ? 1 : 0)
                    VerifiyGuaranteeView()
                        .environmentObject(tabData)
                        .opacity(tabData.currentTab == resellerTabs[2] ? 1 : 0)
                    HistoryView()
                        .opacity(tabData.currentTab == resellerTabs[3] ? 1 : 0)
                    MoreView(isValidAccessToken: $isValidAccessToken)
                        .opacity(tabData.currentTab == resellerTabs[4] ? 1 : 0)
                }
            }
            HStack(spacing: 0) {
                ForEach(resellerTabs, id: \.self) { tab in
                    tabButton(title: tab, showSendSheet: $showSendSheet, showWalletSheet: $showWalletSheet)

                    if tab != resellerTabs.last {
                        Spacer(minLength: 0)
                    }
                }
            }
